...
For those curious to visualize the setup right now, you can check out the arrangement by following this link <a href="https://www.youtube.com/watch?v=JTYFYtZJXro&t=56s" target="_blank">here</a>. The video not only showcases the microphone configuration but also offers a glimpse into the aSCR method. As you watch, you'll be treated to audio recordings of the Cape Moss frogs, along with a probable region from which each call originated as estimated by an aSCR model.
...
Remember the estimate of density we get here is calling density - the number of calls per unit time, per unit area. The default units for the $\texttt{acre()}$ package is to estimate calling density in per min per hectare. So our model estimated about 3661 calls ha$^{-1}$ min$^{-1}$. 

If we want to convert calling density to animal density, we need to know the call rate of the species. We can use estimates of the average call rate from previous studies or we can collect independent data to estimate it ourselves. For the latter, you'd have to monitor several individuals separately from the main aSCR survey to estimate a call rate for each and take the average of those. 

This was done for eight frogs for which the call rates are contained in the object labelled "call_rates". The call rates are the number of calls per minute and the average of all eight is 16.25 calls per individual per minute. 

```{r call_rate, exercise=TRUE,exercise.eval=FALSE}

call_rates # per minute 

ave_call_rate <- mean(call_rates)

```

Converting to animal density is easy, we simply divide the estimated calling density by the average call rate! Try it below. You should get an a value of about 225 animals per hectare. 
...
First, we're going to create a new acre_data object which includes the call rates we used previously and another argument called **survey.length**. This argument scales our calling density estimate and should have the same time unit as the survey length of each call rate sample. Our samples are per minute, so we could specify survey.length as equal to one and estimate calls ha$^{-1}$ min$^{-1}$ as before. 

By including these two additional arguments, our model will automatically convert our calling density estimate to animal density. We could have done this for model1 as well, but we thought it would be good to understand how calling density is converted to animal density using average call rate. 
