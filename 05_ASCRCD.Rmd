...
If you want, you can play around with the dataframe, use functions like tail() to look at the last few detections or try to determine the number of calls detected (see the hint for code).

The "ID" column contains unique identification numbers for all the detected calls, and the column "trap" includes the number of the detector at which the call was recorded. For example, one call with ID "1" was only detected at the second trap. Call "2" was detected at traps three, four and five. Note that the calls aren't labelled in the order they were recorded; we've just used these labels for simplicity.
...
An alternative way is using the function $\texttt{suggest.buffer()}$, also from $\texttt{secr}$. As its name implies, this function provides a mere **suggestion** of buffer distance based on our detection data and traps. The methodology behind the function is not well documented. We're not going to try and explain it here. Still, we can use this function to get a suitable buffer distance and check whether the distance was wide enough after model fitting.
...
Back to the mask! Below, we've created a simple mask with the suggested buffer distance of 14 metres. We visualise the mask and detectors together with ggplot. Inspect the mask object further. Is it a dataframe or a matrix?
...
If you've inspected the object, you would have seen that the created mask object is a matrix. We have to convert it to a dataframe to plot it with ggplot. The code is slightly more complicated than the previous plot, we've colour coded the points using the arguments **color** in the aes functions of each geom_point, and we've assigned a colour to each using the $\texttt{scale_color_manual()}$ function. Lastly, a title is added, and the legend title is removed.

### $\texttt{acre}$ formatting

In the next phase, $\texttt{acre}$ formatting, we supply all the objects from the first phase to a key function of the package called $\texttt{read.acre()}$. This function creates an object of class "acre_data" and essentially links our detectors and detection data together. Any other data objects we need for model fitting are also included here.
...
Finally, we are ready to fit some aSCR models! We have gone through the first two phases, preparing our data objects and putting them together with the $\texttt{read.acre()}$ function. Now, we use the key function, $\texttt{fit.acre()}$, to fit the aSCR model.
...
When you run the code, the output will show several lines starting with "outer mgc"; this is the model finding the best estimates given our data. 
...
We can look at the model estimates now that we have fitted a model. Extracting information from our model object is not different from any other model objects you might have used before. We can use the $\texttt{summary()}$ function to look at a summary of our model estimates:
...
Similarly, we extract the confidence intervals using the function $\texttt{confint()}$. For both, we have to add the argument **type** and assign the character "fitted" to it. This is to ensure the standard errors and confidence intervals are on the same scale as our estimates (more on this later). 

```{r stder, exercise=TRUE,exercise.eval=FALSE}
acre::stdEr(model1_boot, type = "fitted") # standard errors
...
Remember, the estimate of density we get here is calling density - the number of calls per unit time, per unit area. The default units for the $\texttt{acre()}$ package is to estimate calling density per hectare and then whatever we specified for **survey.length**. We didn't assign anything to **survey.length**, so it is taken to be one (i.e. **survey.length = 1**). Consequently, our density estimate is the number of calls per hectare per whatever our survey length was, here it is per 25 seconds. We estimated about 3661 calls ha$^{-1}$ 25s$^{-1}$.

If we want to convert calling density to animal density, we need to know the average call rate of the species. We can use estimates of the average call rate from previous studies or collect independent data to estimate it ourselves. For the latter, you'd have to monitor several individuals separately from the main aSCR survey to estimate a call rate for each and take the average of those.

This was done for eight frogs, and the call rates are contained in the object labelled "call_rates". Here, the call rates are per 25 seconds. For example, one frog called roughly 7 times per 25 seconds. If we take the mean of all eight call rates, we get 6.77 calls per individual per 25 seconds. 
...
Converting to animal density is easy; we divide the estimated calling density by the average call rate! However, we have to be careful about the time units here. Make sure the time units will cancel out so you get the number of animals per distance unit only. Both our calling density estimate and average call rate are per 25 seconds, so we are good to go. What if our density estimate was per second? Then, you would simply have to divide the call rates by 25 and take the mean to get per individual per second. 

Using the call average call rate we calculated above, convert the calling density estimate to animal density below. You should get a value of about 541 animals per hectare.
...
1. Fill in the appropriate value for the argument **survey.length** below (remove the underscores).

```{r exercise1_1, exercise=TRUE,exercise.eval=FALSE}
...
```{r exercise1_2, exercise=TRUE,exercise.eval=FALSE}

```

```{r exercise1_2-solution}
...
3. Obtain appropriate variance estimates using the bootstrap procedure using N = 5 and save the output as "model2_boot".

```{r exercise1_3, exercise=TRUE,exercise.eval=FALSE} 

```

```{r exercise1_3-solution} 
model2_boot <- boot.acre(model2, N = 5) # bootstrapping

acre::stdEr(model2_boot, type = "fitted") # standard error
...
Below we fit a model with a spatial trend in density. The only new thing is the **par_extend_argument**, which requires putting the density model in a list.
...
When we include habitat or spatial covariates in our density model, $\texttt{fit.acre()}$ automatically models density as a function of these variables using a generalised linear model. If you'd like to know more about this statistical method, which is super straightforward to understand, look at the resources on the welcome page. All you need to know for now is that the intercept parameter represents the average calling density when the habitat covariate is zero, i.e., when the x coordinate equals zero. Whether or not this is valuable information depends on the habitat covariate; here, it's not very informative. 

The default for the regression model is to use the so-called log link function. In other words, the log of calling density is modelled as a function of the covariates. We can convert the parameters back to their normal scale by exponentiating them. 

```{r backtrans, exercise=TRUE,exercise.eval=FALSE}

exp(model3$coefficients[5]) # intercept 
exp(model3$coefficients[6]) # effect 

```

Our model estimated roughly 534 frogs per $m^2$ when x = 0. The second parameter, "D.x", is the expected change in animal density per one-unit increase in the habitat covariate. Our model estimated that, on average, animal density increased by roughly 1 frog for every one-metre increase in the x direction. If this value was negative, we would have had a decrease in density for a one-unit increase in the x-direction. 

2. Plot the estimated calling density surface.
