...
When you run the code, the output will show several lines starting with "outer mgc", this the model finding the best estimates given our data. 
...
Using the call average call rate we calculated above, convert the calling density estimate to animal density below. You should get an a value of about 541 animals per hectare.
...
The default for the regression model is to use the so-called log link function. In other words, the log of calling density is modeled as a function of the covariates. We can convert the parameters back to their normal scale by exponentiating them. 
